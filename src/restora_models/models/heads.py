"""Shared output heads for restora-models.

Two heads:
- `DualOutputHead` — the structural colorization fix (Lab `ab` head
  gated by config[0]); deterministic, runs on every output.
- `AdversarialRefineHead` — optional second-pass refinement that takes
  the deterministic output + backbone features and produces a residual
...
class AdversarialRefineHead(nn.Module):
    ...
    Parameter budget (at hidden_dim=96, n_blocks=6, feat_dim=64):
      ...
      Total: ~6.5M params, ~5ms forward at 256² on H200.
    """
